...
class NoduleCharstics:
    ...
    def __str__(self):
        strng = "subtlty (%d) intstruct (%d) calci (%d) sphere (%d) margin (%d) lob (%d) spicul (%d) txtur (%d) malig (%d)" % \
        (self.subtlety, self.internalstructure, self.calcification, self.sphericity, self.margin, self.lobulation, \
        self.spiculation, self.texture, self.malignancy)
        return strng

    def setValues(self, sub, inter, calc, spher, lob, spic, tex, malig):
        self.subtlety = sub
        self.internalstructure = inter
...
class NoduleRoi:   #is common for nodule and nonnodule
    ...
    def __str__(self):
        ...
        strng = "Inclusion (%s) Z = %.2f SOP_UID (%s) \n ROI points [ %d ]  ::  "%(self.inclusion,self.z, self.sop_uid,n_pts)

        if (n_pts > 2):
            strng += "[[ %d,%d ]] :: "%(self.roi_centroid[0],self.roi_centroid[1])
            strng += "(%d, %d), (%d,%d)..."%(self.roi_xy[0][0],self.roi_xy[0][1],self.roi_xy[1][0],self.roi_xy[1][1])
            strng += "(%d, %d), (%d,%d)"%(self.roi_xy[-2][0],self.roi_xy[-2][1],self.roi_xy[-1][0],self.roi_xy[-1][1])
        else:
            for i in range(n_pts):
                strng += "(%d, %d),"% (self.roi_xy[i][0], self.roi_xy[i][1])
        return strng


class Nodule:   #is base class for all nodule types (NormalNodule, SmallNodule, NonNodule)
    def __init__(self):
        self.id = None
        self.rois = []
        self.is_small = False
...
class NoduleAnnotationCluster():   # to be seen
    ...
    def compute_centroid(self):
        ...
        xc = 0.5*(self.convex_hull[0][0] + self.convex_hull[3][0])  # (x_min + x_max)/2
        yc = 0.5*(self.convex_hull[0][1] + self.convex_hull[3][1])  # (y_min + y_max)/2
        self.centroid = (xc,yc)
    ...
    def set_convex_hull(self):
        ...
        self.convex_hull = [(x_min,y_min),(x_max,y_min),
...
class NormalNodule(Nodule): 
    ...
    def __init__(self):
        ...
        self.characterstics = NoduleCharstics()
        self.is_small = False
        
    def tostring(self):
        strng = str(self.characterstics)
        strng += "\n"
